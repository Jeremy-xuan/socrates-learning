...
# -*- coding: utf-8 -*-
"""
使用 Qwen 视觉模型 API 识别扫描版 PDF
陛下旨意：必须使用视觉模型，抽检通过率 100%
"""
import os
import sys
import base64
...
# 多模型列表（按优先级），自动切换
VISION_MODELS = [
    "qwen-vl-ocr-latest",      # 首选：OCR 专用
    "qwen-vl-plus-latest",     # 备选 1：视觉理解
    "qwen3-vl-235b-a22b-thinking"  # 备选 2：深度思考
]
...
def log_api_call(model, page, status, pdf_name, latency_ms=0):
    """记录 API 调用"""
    api_log.append({
...
def recognize_with_vision(image_path, pdf_name, page_num):
    """使用视觉模型识别图片（支持多模型自动切换）"""
    global current_model_index
    
    start_time = time.time()
    tried_models = []
    
    while current_model_index < len(VISION_MODELS):
        model = VISION_MODELS[current_model_index]
        ...
        try:
            ...
            if response.status_code == 200:
                log_api_call(model, page_num, "success", pdf_name, latency_ms)
                # 记录模型切换（如果切换过）
                if len(tried_models) > 1:
                    print(f"[模型切换] {tried_models} → {model}")
                return response.output.choices[0].message.content[0]["text"]
            else:
                log_api_call(model, page_num, f"error_{response.code}", pdf_name, latency_ms)
                # 检查是否额度不足
                if "AllocationQuota" in str(response.code) or "quota" in str(response.message).lower():
                    print(f"  ⚠️ {model} 额度不足，切换到下一个模型...")
            ...
            latency_ms = int((time.time() - start_time) * 1000)
            log_api_call(model, page_num, f"exception_{str(e)[:50]}", pdf_name, latency_ms)
            print(f"  识别错误：{e}")
            return None
    
    # 所有模型都尝试过了
    print(f"  ❌ 所有模型都已尝试：{tried_models}")
    return None
...
def process_pdf(pdf_path, output_md_path):
    ...
    # 创建临时目录存放图片
    temp_dir = os.path.join(os.path.dirname(output_md_path), "temp_vision_images")
    ...
    for i, img_path in enumerate(image_paths, 1):
        ...
        text = recognize_with_vision(img_path, fname, i)
...
def save_api_log(output_dir):
    """保存 API 调用日志"""
    log_path = os.path.join(output_dir, "vision_api_log.json")
...
def main():
    if len(sys.argv) < 3:
        print("用法：python3 convert_pdfs_vision.py <input_dir> <output_dir>")
        sys.exit(1)
    ...
    print(f"使用模型：{VISION_MODELS[0]}（支持自动切换）")
    print(f"备用模型：{', '.join(VISION_MODELS[1:])}")
    print("=" * 60)
    ...
    report_path = os.path.join(output_dir, "vision_conversion_report.json")
