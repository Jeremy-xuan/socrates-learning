...
# Find 2022 directory
dirs_2022 = glob.glob(os.path.join(BASE_DIR, "*2022*md_vision"))
OUTPUT_DIR = dirs_2022[0] if dirs_2022 else None

# Find PDF
pdf_dir = os.path.join(BASE_DIR, "练习册")
pdf_files = glob.glob(os.path.join(pdf_dir, "*2022*.pdf"))
PDF_PATH = pdf_files[0] if pdf_files else None

def main():
    if not OUTPUT_DIR:
        print("错误：未找到 2022 输出目录")
        return
    if not PDF_PATH:
        print("错误：未找到 2022 PDF 文件")
        return
    ...
    # 加载失败页
    failed_path = os.path.join(OUTPUT_DIR, "failed_pages_2022_round1.json")
    ...
    temp_dir = os.path.join(OUTPUT_DIR, "temp_fix_2022")
    ...
    # 读取现有 MD，保留成功页
    existing_md_files = glob.glob(os.path.join(OUTPUT_DIR, "*.md"))
    existing_md = existing_md_files[0] if existing_md_files else None
    
    if not existing_md:
        print("错误：未找到现有 MD 文件")
        return
    ...
    for i, fail_info in enumerate(failed_pages):
        page_num = fail_info["page"]
        ...
        print(f"\n[{i+1}/{len(failed_pages)}] 修复第 {page_num} 页...", end=" ", flush=True)
    ...
    # 重新生成 MD
    md_content = "# 物理电磁 C 练习册-2022 北美 FRQ 真题-P417-P474\n\n"
    ...
    quality_report = {
        ...
        "success_rate": (success_count / 58) * 100,
        "page_stats": quality_stats,
        "fixed_pages": fixed_count,
        "original_failed": len(failed_pages)
    }
    
    with open(os.path.join(OUTPUT_DIR, "quality_report_2022.json"), "w", encoding="utf-8") as f:
        json.dump(quality_report, f, indent=2, ensure_ascii=False)
    
    # 更新重试页报告
    with open(os.path.join(OUTPUT_DIR, "rerun_pages_2022_round2.json"), "w", encoding="utf-8") as f:
        json.dump(rerun_results, f, indent=2, ensure_ascii=False)
    
    # 保存 API 日志（第三轮）
    with open(os.path.join(OUTPUT_DIR, "vision_api_log_2022_round3.json"), "w", encoding="utf-8") as f:
        json.dump(api_log, f, indent=2, ensure_ascii=False)
    
    # 更新抽检表
    import random
    sample_pages = sorted(random.sample(range(1, 59), min(10, 58)))
    sample_table = []
    ...
    with open(os.path.join(OUTPUT_DIR, "sample_audit_2022_10pages.json"), "w", encoding="utf-8") as f:
        json.dump({
    ...
    print(f"总成功率：{success_count}/58 ({quality_report['success_rate']:.1f}%)")
